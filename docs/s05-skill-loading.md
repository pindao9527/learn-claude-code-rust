...
registry.insert("code-review".to_string(), code_review_doc);
registry.insert("git-workflow".to_string(), git_workflow_doc);
```
...
pub struct SkillRegistry {
    pub skills: HashMap<String, SkillDocument>,
}

impl SkillRegistry {
    pub fn load_all(skills_dir: &std::path::Path) -> Result<Self, SkillError> {
        let mut skills = HashMap::new();
        // 借助 walkdir 进行安全的递归访问
        for entry in walkdir::WalkDir::new(skills_dir).into_iter().filter_map(|e| e.ok()) {
            if entry.file_name() == "SKILL.md" {
                ...
                let name = meta.get("name").cloned()
                    .unwrap_or_else(|| entry.path().parent().unwrap().file_name().unwrap().to_string_lossy().to_string());
                ...
                skills.insert(name, SkillDocument { manifest, body });
            }
        }
        Ok(Self { skills })
    }
...
```rust
let system_prompt = format!("You are a coding agent.
Skills available:
...
let output = match tool_name {
    "load_skill" => {
        ...
        match skill_registry.load_full_text(name) {
            Ok(body) => body,
